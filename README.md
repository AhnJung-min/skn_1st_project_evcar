...
**안정민**
> 많이 해보지 않은 팀장 역할을 맡으면서, 초반에 방향을 제대로 잡지 못한 탓에 시간을 적지 않게 흘려보냈습니다. 그 경험이 오히려 팀원들이 혼란 없이 움직일 수 있도록 기틀부터 잡아야 한다는 것을 몸소 깨닫게 해주었습니다. 짧은 프로젝트 안에서 수업 시간에 배운 기술을 직접 시각화로 구현해냈다는 점도 작지 않은 성취였고, 크고 작은 어려움 속에서도 팀원 모두가 끝까지 함께 해준 덕분에 뿌듯한 결과를 만들어낼 수 있었습니다. 이번 경험이 다음 프로젝트에서는 팀과 더 잘 맞춰 나가는 밑거름이 될 것 같습니다.

**신가을**
> 프로젝트 진행 과정에서 발생한 사안들이 기존과 다른 생소한 부분들이어서 다소 어려움이 있었다. 프로젝트가 완료된 현재 시점에서 돌이켜보면 교육과 실습을 통해 배웠던 일련의 과정들을 활용하면 해결이 가능한 내용들이었다.
>
> 최초 프로젝트 기획 단계에서 협업을 위한 git 설정 및 DB 설계를 명확히 연계하지 못해, 주제 선정과 데이터 수집 이후에는 어디서부터 무엇을 해야 하는지 갑자기 머리가 비어버린 상황이 가장 난감한 부분이었다. 팀원들과 함께 프로젝트를 진행하면서 각 단계별 필요한 사항에 대해서 조금씩이나마 인지하면서 진행한 것이 가장 큰 수확이 아닐까 생각한다.
>
> 기획 단계에서 프로젝트 추진을 위한 협업 등 기초환경 구성, 추진 단계에서 DB 설계와 DB 적재 과정, streamlit을 통한 화면 구성, 그리고 각자가 작업한 내용을 최종적으로 취합하면서 발생한 문제 해결 등 전반적인 흐름을 조금이나마 이해할 수 있었다.
>
> 다만, git 설정 및 github branch/merge 등 일부 직접 해보고 경험해보지 못한 부분과 작성한 코드를 정확하게 이해하기 보다 AI를 활용하여 단순히 문제점을 해결하는 것으로 국한된 부분들이 다소 아쉬움을 가졌다. 이런 경험을 토대로 향후에는 더 좋은 프로젝트를 추진할 수 있도록 해야겠다.

**권세진**
> 프로젝트 초반에는 소통의 아쉬움으로 데이터 적재가 분산되어 ERD 완성이 다소 늦어지는 시행착오를 겪었지만 이 과정 속에서 초기 소통과 데이터 설계가 얼마나 중요한지 배울 수 있었습니다. 막막했던 시작 단계에서 길을 잡아주신 팀장님 덕분에 프로젝트를 진행할 수 있었고, 발표 직전 마무리가 다급했던 순간에는 팀원들의 도움으로 무사히 프로젝트를 마칠 수 있었습니다. 끝까지 열정적으로 참여해 준 팀원들을 만난 덕분에 즐거웠습니다!

**김길환**
> 프로젝트를 첫 회의때 머릿속에 가이드라인도 잡히지 않아 걱정이 많았는데, 팀장님인 정민님을 필두로 팀원분들이 잘 끌어주셔서 결과물을 낼 수 있었다고 생각합니다. 개인적으로는 결과물이 있다는 것에 만족감을 느끼는 것 같습니다.
>
> 코드를 짜고 구현하는 것은 AI와 함께 바이브 코딩으로 하다보니 할만했는데, 오류가 났을때 해결하는 것이나 환경변수를 설정하는 기초 지식들이 중요하다는 생각을 많이 했습니다.

**김혜리**
> 첫 프로젝트인 만큼 걱정이 정말 많았는데 팀장님 덕분에 잘 끝낼 수 있었던 것 같습니다. 개인적으로 아쉬운 점이 많은 것 같은데 가장 아쉬운 점은 대량의 데이터를 한번에 사용하려고 했던 점입니다. 데이터에 대한 이해가 부족했던 것 같습니다. 또, 학교에서 해오던게 있어서 자꾸 분석에 맞춰 생각을 하게 되어서 정보 조회 프로그램이 아닌 분석 결과를 보여주게 된 것 같아 아쉽기도 합니다. 아쉬운만큼 얻어가는 것도 많았던 프로젝트였던 것 같습니다. 수업 때 배웠던 것들을 직접 해보면서 매커니즘을 이해할 수 있게 되었고, 또 db설계가 정말 중요하다는 걸 깨달았습니다.
